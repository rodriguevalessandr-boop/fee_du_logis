...
window.sauvegarderNotif = () => {
    ...
    if (heure) {
        state.heureNotif = heure;
        sauvegarder();
        planifierRappelQuotidien(heure); // On lance la planification
        document.getElementById('notif-modal').classList.add('hidden');
        alert(`Rappel magique réglé sur ${heure} ! ✨`);
    }
};
...
function planifierRappelQuotidien(heure) {
    ...
    navigator.serviceWorker.ready.then(reg => {
        if (reg.active) {
            ...
            console.log("Sortilège de rappel programmé pour :", cible.toLocaleString());
        }
    });
}